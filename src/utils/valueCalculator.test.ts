...
describe("valueCalculator", () => {
    ...
    it("handles string estimatedValue (treats as 0)", () => {
      const items = [
        ...
        },
        {
          id: "2",
          category: "tv",
          estimatedValue: "expensive" as unknown as number,
          status: "not_covered",
          color: "red",
          source: "detected" as const,
        },
  ...
  describe("createItemBreakdown", () => {
    ...
    it("uses default value when manual item has no estimatedValue", () => {
      const manualItems = [{ id: "m1", name: "My Laptop", category: "laptop" }] as ManualItem[];
    ...
    it("skips items without id", () => {
      const detectedItems = [{ category: "laptop", confidence: 0.9 }] as DetectedItem[];
    ...
    it("skips items without category", () => {
      const detectedItems = [{ id: "1", confidence: 0.9 }] as DetectedItem[];
    ...
    it("handles null/undefined inputs", () => {
      const result = createItemBreakdown(
        null as unknown as DetectedItem[],
        null as unknown as ManualItem[],
        "renters",
      );
      expect(result).toEqual([]);
    });
