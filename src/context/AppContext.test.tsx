...
function TestConsumer(): ReactElement {
  ...
  return (
    <div>
      ...
      <button data-testid="completeOnboarding" onClick={() => context.completeOnboarding()}>
        Complete
      </button>
      <button data-testid="setActiveTab" onClick={() => context.setActiveTab("dashboard")}>
        Set Tab
      </button>
      <button data-testid="setSelectedItem" onClick={() => context.setSelectedItem("item-1")}>
        Select Item
      </button>
      <button data-testid="clearSelectedItem" onClick={() => context.setSelectedItem(null)}>
        Clear Item
      </button>
      <button
        data-testid="updateDetectedItems"
        onClick={() => {
      ...
      <button
        data-testid="addManualItem"
        onClick={() =>
          context.addManualItem({
            id: "manual-1",
            name: "Watch",
            category: "Jewelry",
            estimatedValue: 500,
      ...
      <button data-testid="removeManualItem" onClick={() => context.removeManualItem("manual-1")}>
        Remove Manual
      </button>
      <button
        data-testid="updateManualItem"
        onClick={() =>
          context.updateManualItem("manual-1", { name: "Updated Watch", estimatedValue: 600 })
        }
      >
        Update Manual
      </button>
      <button
        data-testid="setConfidenceThreshold"
...
function renderWithProvider(
  ui: ReactElement,
  { providerProps = {} }: { providerProps?: Partial<ProviderProps> } = {},
...
describe("AppContext", () => {
  ...
  describe("initial state", () => {
    ...
    it("loads manualItems from localStorage if available", () => {
      const manualItems: ManualItem[] = [
        { id: "item-1", name: "Jewelry", category: "Jewelry", estimatedValue: 1000 },
      ];
      ...
      function PolicyTest(): ReactElement {
        ...
        return (
          <div>
            ...
            <button data-testid="set-renters" onClick={() => setPolicyType("renters")}>
              Set Renters
            </button>
            <button data-testid="set-homeowners" onClick={() => setPolicyType("homeowners")}>
              Set Homeowners
            </button>
            <button data-testid="set-auto" onClick={() => setPolicyType("auto")}>
              Set Auto
            </button>
            <button data-testid="set-none" onClick={() => setPolicyType("none")}>
              Set None
            </button>
          </div>
        );
      ...
      function ThresholdTest(): ReactElement {
        ...
        return (
          <div>
            ...
            <button data-testid="setLow" onClick={() => setConfidenceThreshold(0.05)}>
              Set Low
            </button>
          </div>
        );
    ...
    it("normalizes values above maximum to 0.9", () => {
      function ThresholdTest(): ReactElement {
        ...
        return (
          <div>
            <div data-testid="threshold">{confidenceThreshold}</div>
            <button data-testid="setHigh" onClick={() => setConfidenceThreshold(0.95)}>
              Set High
            </button>
          </div>
        );
      ...
      function ThresholdTest(): ReactElement {
        ...
        return (
          <div>
            <div data-testid="threshold">{confidenceThreshold}</div>
            <button
              data-testid="setInvalid"
              onClick={() => setConfidenceThreshold("invalid" as unknown as number)}
            >
              Set Invalid
            </button>
          </div>
    ...
    it("accepts all valid threshold values", () => {
      function ThresholdTest(): ReactElement {
        ...
        return (
          <div>
            <div data-testid="threshold">{confidenceThreshold}</div>
            <button data-testid="set10" onClick={() => setConfidenceThreshold(0.1)}>
              Set 10%
            </button>
            <button data-testid="set25" onClick={() => setConfidenceThreshold(0.25)}>
              Set 25%
            </button>
            <button data-testid="set50" onClick={() => setConfidenceThreshold(0.5)}>
              Set 50%
            </button>
            <button data-testid="set75" onClick={() => setConfidenceThreshold(0.75)}>
              Set 75%
            </button>
            <button data-testid="set90" onClick={() => setConfidenceThreshold(0.9)}>
              Set 90%
            </button>
          </div>
        );
        ...
        return (
          <div>
            ...
            <button
              data-testid="update"
              onClick={() =>
                updateManualItem("manual-1", { name: "Updated Watch", estimatedValue: 600 })
              }
            >
              Update
            </button>
  ...
  describe("localStorage persistence edge cases", () => {
    ...
    it("multiple state changes accumulate in localStorage", () => {
      function MultiChangeTest(): ReactElement {
        ...
        return (
          <div>
            <button data-testid="setHomeowners" onClick={() => setPolicyType("homeowners")}>
              Homeowners
            </button>
            <button
              data-testid="addItem1"
              onClick={() =>
                addManualItem({ id: "m1", name: "Ring", category: "Jewelry", estimatedValue: 1000 })
              }
            >
              Add Ring
            </button>
            <button
              data-testid="addItem2"
              onClick={() =>
                addManualItem({
                  id: "m2",
                  name: "Necklace",
  ...
  describe("useAppContext hook", () => {
    it("throws when used outside of provider", () => {
      ...
      const consoleSpy: MockInstance<(...args: unknown[]) => void> = vi
        .spyOn(console, "error")
        .mockImplementation(() => {});
