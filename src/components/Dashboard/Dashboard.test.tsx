...
import type { DashboardProps, DetectedItem, ItemBreakdown, ManualItem } from "../../types";
...
const mockedGetUpgradeRecommendations =
  getUpgradeRecommendations as Mock<GetUpgradeRecommendationsFn>;
...
describe("Dashboard", () => {
  ...
  describe("empty state", () => {
    ...
    it("shows helpful message when no items detected", () => {
      ...
      expect(
        screen.getByText(/point your camera at objects to begin scanning/i),
      ).toBeInTheDocument();
    });
  ...
  describe("full coverage", () => {
    it("shows all items covered with positive message", () => {
      ...
      const props: DashboardProps = {
        detectedItems: [createDetectedItem("1", "laptop"), createDetectedItem("2", "tv")],
        manualItems: [],
  ...
  describe("mixed coverage", () => {
    ...
    it("includes manual items in calculations", () => {
      ...
      const manualItems: ManualItem[] = [createManualItem("m1", "Expensive Watch", "clock", 5000)];
  ...
  describe("rapid updates", () => {
    it("handles rapid item additions without NaN", () => {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        const updatedProps: DashboardProps = {
          detectedItems: Array(i + 1)
            .fill(null)
            .map((_, idx) => createDetectedItem(`${idx}`, "laptop")),
          manualItems: [],
          policyType: "renters",
          onItemClick: vi.fn(),
        };
    ...
    it("handles negative values gracefully by treating as zero", () => {
      mockedCalculateValues.mockReturnValue({
        ...
        unprotectedValue: 0,
        coverageGapPercentage: 0.0,
        items: [createItemBreakdown("1", "item", 1000, "covered", "green", "detected")],
      });
      mockedGetUpgradeRecommendations.mockReturnValue([]);
  ...
  describe("upgrade recommendations", () => {
    it("shows recommendations when items are uncovered", () => {
      ...
      const props: DashboardProps = {
        detectedItems: [createDetectedItem("1", "laptop"), createDetectedItem("2", "car")],
        manualItems: [],
        policyType: "renters",
        onItemClick: vi.fn(),
      };
