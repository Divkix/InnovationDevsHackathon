...
import { AlertTriangle, Car, CheckCircle, Home, Shield, ShieldAlert, XCircle } from "lucide-react";
import { useCallback, useMemo } from "react";
import { useAppContext } from "@/context/AppContext";
import { VALID_POLICY_TYPES } from "@/utils/coverageLookup";
import { calculateValues, formatCurrency, formatPercentage } from "@/utils/valueCalculator";
import type { DetectedItem, ManualItem, PolicySelectorProps, PolicyType } from "../../types";

/**
 * Policy configuration type definition
 */
...
export function PolicySelector({
  ...
  manualItems = [],
  variant = "default",
}: PolicySelectorProps): React.JSX.Element {
  ...
  const coverageValues = useMemo(() => {
    const safeDetectedItems: DetectedItem[] = Array.isArray(detectedItems) ? detectedItems : [];
    const safeManualItems: ManualItem[] = Array.isArray(manualItems) ? manualItems : [];
  ...
  const handleKeyDown = useCallback(
    (event: React.KeyboardEvent<HTMLButtonElement>, policyTypeValue: PolicyType): void => {
      if (event.key === "Enter" || event.key === " ") {
        event.preventDefault();
        handlePolicySelect(policyTypeValue);
      }
    },
    [handlePolicySelect],
  );

  const { totalValue, protectedValue, unprotectedValue, coverageGapPercentage } = coverageValues;
    ...
    return (
      ...
      >
        <div className="flex items-center gap-2 bg-white/90 backdrop-blur-sm rounded-lg px-3 py-2 shadow-sm border border-gray-200">
          ...
          <span className="text-sm font-medium text-gray-700">Current Policy:</span>
   ...
   */
  return (
    <div data-testid="policy-selector" className="policy-selector w-full max-w-2xl mx-auto">
        ...
        <div className="p-4">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="text-center">
              ...
              <p data-testid="total-value" className="text-xl font-bold text-gray-900">
            ...
            {/* Protected Value */}
            <div className="text-center">
              <p className="text-xs text-gray-500 uppercase tracking-wide mb-1">Protected</p>
              <p
            ...
            {/* Coverage Gap */}
            <div className="text-center">
              <p className="text-xs text-gray-500 uppercase tracking-wide mb-1">Gap</p>
              <p
