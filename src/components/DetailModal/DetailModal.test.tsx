  ...
  lookupCoverage: vi.fn((category: string, policyType: PolicyType): CoverageResult => {
    ...
    if (category === "bicycle") {
      return {
        ...
        conditions: [
          "Bike value may exceed policy sub-limits",
          "High-end bikes need scheduled coverage",
        ],
...
vi.mock("@/data/coverageRules.json", () => ({
  default: {
    renters: {
      ...
      bicycle: {
        commonScenarios: [
          "Theft from home (up to policy limits)",
          "Theft from locked vehicle (up to $200)",
...
describe("DetailModal", () => {
  ...
  describe("Modal Opening", () => {
    it("renders when isOpen is true", () => {
      ...
      renderWithProvider(
        <DetailModal isOpen={true} onClose={mockOnClose} item={item} policyType="renters" />,
      );
    ...
    it("does not render when isOpen is false", () => {
      ...
      renderWithProvider(
        <DetailModal isOpen={false} onClose={mockOnClose} item={item} policyType="renters" />,
      );
  ...
  describe("Covered Item Display", () => {
    ...
    it("shows item name for covered item", () => {
      renderWithProvider(
        <DetailModal isOpen={true} onClose={mockOnClose} item={coveredItem} policyType="renters" />,
      );
  ...
  describe("Conditional Item Display", () => {
    ...
    it("shows how to get full coverage for conditional item", () => {
      ...
      expect(screen.getByTestId("upgrade-options")).toHaveTextContent(
  ...
  describe("Modal Closing", () => {
    ...
    it("closes via X button click", () => {
      renderWithProvider(
        <DetailModal isOpen={true} onClose={mockOnClose} item={modalItem} policyType="renters" />,
      );
  ...
  describe("Camera View Context", () => {
    it("applies dimmed backdrop when source is camera", () => {
      ...
      renderWithProvider(
        <DetailModal isOpen={true} onClose={mockOnClose} item={cameraItem} policyType="renters" />,
      );
  ...
  describe("Policy Change Updates", () => {
    ...
    it("updates content when policyType prop changes", () => {
      const { rerender } = renderWithProvider(
        <DetailModal isOpen={true} onClose={mockOnClose} item={policyItem} policyType="renters" />,
      );
      ...
      rerender(
        <AppProvider>
          <DetailModal isOpen={true} onClose={mockOnClose} item={policyItem} policyType="auto" />
        </AppProvider>,
      );
    ...
    it("shows updated coverage note when policy changes", () => {
      ...
      const { rerender } = renderWithProvider(
        <DetailModal isOpen={true} onClose={mockOnClose} item={carItem} policyType="renters" />,
      );
      ...
      rerender(
        <AppProvider>
          <DetailModal isOpen={true} onClose={mockOnClose} item={carItem} policyType="auto" />
        </AppProvider>,
      );
  ...
  describe("Edge Cases", () => {
    ...
    it("handles unknown categories gracefully", () => {
      ...
      renderWithProvider(
        <DetailModal isOpen={true} onClose={mockOnClose} item={unknownItem} policyType="renters" />,
      );
    ...
    it('shows "No Insurance" when policyType is none', () => {
      ...
      renderWithProvider(
        <DetailModal isOpen={true} onClose={mockOnClose} item={laptopItem} policyType="none" />,
      );
