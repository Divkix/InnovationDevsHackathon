...
describe("CoverageOverlay", () => {
  ...
  describe("geometry helpers", () => {
    ...
    it("projects video-space boxes into displayed canvas space", () => {
      ...
      expect(
        projectBoundingBoxToCanvas({ originX: 100, originY: 80, width: 50, height: 40 }, layout),
      ).toEqual({
        x: 200,
        y: 40,
        width: 100,
  ...
  beforeEach(() => {
    ...
    Object.defineProperty(HTMLCanvasElement.prototype, "height", {
      ...
      set: (value: number) => {
        mockCanvasElement.height = value;
      },
    });
  });
  ...
  describe("color coding", () => {
    it("renders green box with checkmark for covered item", () => {
      ...
      const { container } = render(
        <CoverageOverlay videoRef={mockVideoRef} detections={detections} policyType="renters" />,
      );
    ...
    it("renders red boxes for all items when No Insurance policy selected", () => {
      ...
      const detections: Detection[] = [
        ...
        {
          categories: [{ categoryName: "tv", score: 0.85, displayName: "TV" }],
          boundingBox: { originX: 400, originY: 100, width: 180, height: 120 },
        },
        {
          categories: [{ categoryName: "couch", score: 0.8, displayName: "Couch" }],
          boundingBox: { originX: 100, originY: 300, width: 250, height: 150 },
        },
      ];

      const mockVideoWithReady = {
        current: { ...createMockVideoElement(), readyState: 2 } as HTMLVideoElement,
      };

      const { container } = render(
        <CoverageOverlay videoRef={mockVideoWithReady} detections={detections} policyType="none" />,
      );
  ...
  describe("multi-object handling", () => {
    it("renders separate boxes for multiple detections", () => {
      ...
      const detections: Detection[] = [
        ...
        {
          categories: [{ categoryName: "car", score: 0.9, displayName: "Car" }],
          boundingBox: { originX: 500, originY: 100, width: 300, height: 200 },
        },
        {
          categories: [{ categoryName: "bicycle", score: 0.85, displayName: "Bicycle" }],
          boundingBox: { originX: 100, originY: 350, width: 150, height: 100 },
        },
      ];
    ...
    it("handles overlapping boxes with semi-transparent fills", () => {
      ...
      const detections: Detection[] = [
        ...
        {
          categories: [{ categoryName: "keyboard", score: 0.85, displayName: "Keyboard" }],
          boundingBox: { originX: 150, originY: 180, width: 150, height: 80 },
        },
      ];
  ...
  describe("empty state", () => {
    it("renders no boxes when no detections", () => {
      const { container } = render(
        <CoverageOverlay videoRef={mockVideoRef} detections={[]} policyType="renters" />,
      );
    ...
    it("clears canvas when detections go from present to empty", () => {
      ...
      rerender(<CoverageOverlay videoRef={mockVideoRef} detections={[]} policyType="renters" />);
  ...
  describe("canvas sizing", () => {
    ...
    it("updates canvas size when video dimensions change", () => {
      ...
      rerender(
        <CoverageOverlay videoRef={newMockVideoRef} detections={detections} policyType="renters" />,
      );
  ...
  describe("policy type changes", () => {
    it("updates colors when policy type changes", () => {
      ...
      // Change policy type
      rerender(
        <CoverageOverlay videoRef={mockVideoWithReady} detections={detections} policyType="auto" />,
      );
  ...
  describe("detection confidence filtering", () => {
    ...
    it("renders boxes only for high confidence detections", () => {
      ...
      const detections: Detection[] = [
        ...
        {
          categories: [{ categoryName: "tv", score: 0.3, displayName: "TV" }],
          boundingBox: { originX: 400, originY: 100, width: 180, height: 120 },
        },
      ];
