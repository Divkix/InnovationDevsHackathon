...
// Mock TensorFlow.js - factory must be self-contained (hoisted)
vi.mock('@tensorflow/tfjs', () => {
  // All mock functions defined INSIDE the factory
  ...
  const mockModel = {
    predict: mockPredict.mockReturnValue(mockTensor),
  ...
  return {
    ...
    // Export mocks for test access via import
    __mockTensorDispose: mockTensorDispose,
    __mockModelDispose: mockModelDispose,
    __mockPredict: mockPredict,
    __mockSetBackend: mockSetBackend,
    __mockLoadGraphModel: mockLoadGraphModel,
    __mockReady: mockReady,
    __mockFromPixels: mockFromPixels,
    __mockResizeBilinear: mockResizeBilinear
  }
})

vi.mock('@tensorflow/tfjs-backend-webgl', () => ({}))

// Access mock functions from module
let mockTensorDispose, mockModelDispose, mockPredict, mockSetBackend, mockLoadGraphModel, mockReady, mockFromPixels, mockResizeBilinear
...
describe('useYOLODetection', () => {
  ...
  describe('model loading', () => {
    ...
    it('loads model from correct path', async () => {
      ...
      await waitFor(() => {
        expect(mockLoadGraphModel).toHaveBeenCalledWith('/models/yolo26n/model.json')
      })
    })
  ...
  describe('error handling', () => {
    it('sets error state when backend initialization fails', async () => {
      mockSetBackend.mockRejectedValueOnce(new Error('Backend initialization failed'))
      ...
      expect(result.current.error.message).toContain('Backend initialization failed')
      expect(result.current.isLoaded).toBe(false)
    })

    it('sets error state when model loading fails', async () => {
      mockLoadGraphModel.mockRejectedValueOnce(new Error('Failed to fetch model'))
