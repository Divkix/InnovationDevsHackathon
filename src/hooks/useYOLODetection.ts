...
export interface UseYOLODetectionReturn {
	...
	detect: (video: HTMLVideoElement | null, timestamp: number) => Promise<{ detections: Detection[] }>;
...
export function useYOLODetection(): UseYOLODetectionReturn {
	...
	const detect = useCallback(
		async (video: HTMLVideoElement | null, _timestamp: number): Promise<{ detections: Detection[] }> => {
			...
			try {
				...
				try {
					...
					const outputTensor =
						results.output0 as ort.Tensor ??
						results[Object.keys(results)[0]] as ort.Tensor;
