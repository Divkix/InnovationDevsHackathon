...
vi.mock("../components/CameraView/CameraView.jsx", () => ({
  CameraView: vi.fn(
    ({ onError, onManualMode }: CameraViewMockProps): ReactElement => (
      <div data-testid="mock-camera-view">
        <p>Camera View Mock</p>
        <button onClick={() => onError?.(new Error("Camera error"))}>Trigger Error</button>
        <button onClick={() => onManualMode?.()}>Use Manual Mode</button>
      </div>
    ),
  ),
}));
...
vi.mock("../components/PolicySelector/PolicySelector.jsx", () => ({
  PolicySelector: vi.fn(
    ({ variant }: PolicySelectorMockProps): ReactElement => (
...
vi.mock("../components/Dashboard/Dashboard.jsx", () => ({
  Dashboard: vi.fn(
    ({ detectedItems, manualItems, policyType }: DashboardMockProps): ReactElement => (
...
vi.mock("framer-motion", () => ({
  motion: {
    ...
    nav: ({
      ...
      children: React.ReactNode;
      [key: string]: unknown;
    }): ReactElement => <nav {...props}>{children}</nav>,
    header: ({
      children,
      ...props
    }: {
      children: React.ReactNode;
      [key: string]: unknown;
    }): ReactElement => <header {...props}>{children}</header>,
    main: ({
      children,
      ...props
    }: {
...
vi.mock("lucide-react", () => ({
  ...
  Link2: () => <svg data-testid="icon-link-2" />,
...
describe("App - Tab Navigation Integration", () => {
  ...
  describe("header and layout", () => {
    ...
    it("shows compact variant of PolicySelector in header", async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTestId("mock-policy-selector")).toHaveTextContent(
          "compact Policy Selector",
        );
      });
    });
